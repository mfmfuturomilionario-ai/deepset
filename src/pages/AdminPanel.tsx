...
import { Shield, Users, Coins, Brain, Plus, Minus, Search, Key, Settings } from 'lucide-react';

const LLM_PROVIDERS = [
  { value: 'lovable', label: 'Lovable AI (Nativa)' },
      ...
      <Tabs defaultValue="users" className="space-y-4">
              ...
              <TableBody>
                {filteredUsers.map(u => (
                  <TableRow key={u.user_id}>
                    ...
                    <TableCell>
                      <span className={`text-xs px-2 py-1 rounded-full ${u.role === 'admin' ? 'bg-primary/20 text-primary' : 'bg-secondary text-muted-foreground'}`}>
                        {u.role}
                      </span>
                    </TableCell>
                    <TableCell className="font-bold">{u.balance}</TableCell>
                    <TableCell>
                      <Switch checked={u.is_active} onCheckedChange={() => toggleUserActive(u.user_id, u.is_active)} />
                    </TableCell>
                    <TableCell>
                      <div className="flex gap-1">
                        <Button size="sm" variant="ghost" onClick={() => handleAddCredits(u.user_id, parseInt(creditAmount) || 10)}>
                          <Plus className="w-3 h-3" />
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => handleRemoveCredits(u.user_id, parseInt(creditAmount) || 10)}>
        ...
        <TabsContent value="credits" className="space-y-4">
          <Card className="glass-card">
            <CardHeader>
              <CardTitle className="text-base font-display">Gerenciar Créditos</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label>Quantidade de créditos por operação</Label>
          ...
          <Card className="glass-card">
            <CardHeader>
              <CardTitle className="text-base font-display">Resumo</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <p className="text-2xl font-bold font-display text-primary">{users.reduce((sum, u) => sum + u.balance, 0)}</p>
                  <p className="text-sm text-muted-foreground">Total créditos ativos</p>
                </div>
                <div className="text-center">
        ...
        <TabsContent value="llm" className="space-y-4">
          <Card className="glass-card">
            <CardHeader>
              <CardTitle className="text-base font-display flex items-center gap-2">
                <Settings className="w-4 h-4" /> LLM Padrão Global
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
          ...
          <Card className="glass-card">
            <CardHeader>
              <CardTitle className="text-base font-display">LLM por Usuário (Override)</CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
        ...
        <TabsContent value="keys" className="space-y-4">
          <Card className="glass-card">
            <CardHeader>
              <CardTitle className="text-base font-display">Adicionar API Key</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                <Select value={newApiKey.provider} onValueChange={v => setNewApiKey(prev => ({ ...prev, provider: v }))}>
                  ...
                  <SelectContent>
                    {LLM_PROVIDERS.filter(p => p.value !== 'lovable').map(p => (
                      <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div>
                <Label>API Key</Label>
                <Input type="password" value={newApiKey.api_key} onChange={e => setNewApiKey(prev => ({ ...prev, api_key: e.target.value }))} placeholder="sk-..." />
              </div>
              <Button onClick={handleSaveApiKey} className="gradient-orange text-primary-foreground">
                <Key className="w-4 h-4 mr-2" /> Salvar API Key
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card">
            <CardHeader>
              <CardTitle className="text-base font-display">Keys Configuradas</CardTitle>
            </CardHeader>
            <CardContent>
              {apiKeys.length === 0 ? (
